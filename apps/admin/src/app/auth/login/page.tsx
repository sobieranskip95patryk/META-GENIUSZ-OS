...
export default function AdminLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (json.user?.role !== "ADMIN" && json.user?.role !== "MODERATOR") {
    ...
    <main className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-black text-gray-900 mt-2">Panel Admin</h1>
          <p className="text-gray-500 text-sm mt-1">Dostęp tylko dla administratorów i moderatorów</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 space-y-4">
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-gray-900 hover:bg-black disabled:opacity-50 text-white font-semibold py-2.5 rounded-xl transition-colors text-sm">
            {loading ? "Logowanie..." : "Zaloguj się"}
          </button>
        </form>

        <p className="text-center text-xs text-gray-400 mt-4">
          <Link href="/" className="hover:text-gray-600">← Powrót do dashboardu</Link>
        </p>
      </div>
    </main>
  );
